...
def get_recommended_restaurants(request):
    try:
        ...
        recommended_restaurants = load_modal.predict_user_cluster(
            preferences=preference_df
        )

        # Pagination
        paginator = Paginator(
            recommended_restaurants.to_dict(orient="records"), 12
...
def preference(request):
    if request.method == "POST":
        ...
        preference = Preference.objects.create(
            price_level=price_level, avg_rating=avg_rating
        )
        ...
        preference.region = data.get("region", "")
        preference.save()
...
def get_regions(request):
    df = DATA_FRAME.copy()
    # Ensure distinct regions per country by dropping duplicates
    distinct_regions = df.drop_duplicates(subset=["country", "region"])

    # Group by 'country' and collect distinct regions into a list under each country
    grouped = (
        distinct_regions.groupby("country")["region"]
    ...
    data = [
        {"country": row["country"], "region": region}
        for _, row in grouped.iterrows()
        for region in row["regions"]
    ]

    # Pagination
    paginator = Paginator(data, 1000)  # 1000
    page_number = request.GET.get("page", 1)
    try:
        page_obj = paginator.page(page_number)
    except PageNotAnInteger:
    ...
    # Constructing the response
    response_data = {
        "data": list(page_obj.object_list),  # The current page's items
        "total_pages": paginator.num_pages,  # Total number of pages
        "current_page": page_number,  # The current page
        "has_next": page_obj.has_next(),  # Boolean indicating if there's a next page
        "has_previous": page_obj.has_previous(),  # Boolean indicating if there's a previous page
...
def plotly_histogram(request):
    ...
    fig = px.bar(
        ...
        labels={"country": "Pays", "y": "Restaurants"},
    )
...
def diet_adaptation(request):
    df = DATA_FRAME.copy()
    df[["vegetarian_friendly", "vegan_options", "gluten_free"]] = df[
        ["vegetarian_friendly", "vegan_options", "gluten_free"]
    ].replace({"Y": True, "N": False})

    df["vegan_gluten_free"] = (df["vegan_options"] == True) & (
        df["gluten_free"] == True
    )
    df["vegetarian_gluten_free"] = (df["vegetarian_friendly"] == True) & (
        df["gluten_free"] == True
    )
    df["vegan_vegetarian"] = (df["vegan_options"] == True) & (
        df["vegetarian_friendly"] == True
    )
    df["vegan_gluten_free_vegetarian"] = (
        (df["vegan_options"] == True)
        & (df["gluten_free"] == True)
        & (df["vegetarian_friendly"] == True)
    )
    ...
    fig = px.bar(
        ...
        labels={
            ...
            "vegan_options": "Options Véganes",
            "gluten_free": "Options sans gluten",
            "vegan_gluten_free": "Options vegan et sans gluten",
            "vegetarian_gluten_free": "Options vegetarienne et sans gluten",
            "vegan_vegetarian": "Options vegan et vegetarienne",
            "vegan_gluten_free_vegetarian": "Options vegan, vegetarienne et sans gluten",
            "value": "Nombre de Restaurants",
        },
...
def popularity_diet(request):
    ...
    df["total_adaptations"] = df[
        ["vegetarian_friendly", "vegan_options", "gluten_free"]
    ].sum(axis=1)

    fig = px.scatter(
        df,
        x="total_reviews_count",
        y="total_adaptations",
        color="avg_rating",
        title="Corrélation entre Popularité et adaptation diététique",
    )
    fig.update_layout(xaxis_title="Nombre d'Avis", yaxis_title="Nombre d'adaptation")
...
def distrib_restaurant_regimes(request):
    ...
    grouped_df = (
        ...
        .agg(
            {
                "total_adaptations": "mean",
                "latitude": "first",
                "longitude": "first",
                "restaurant_link": "size",
            }
        )
        .reset_index()
    )
...
def plotly_bar_chart(request):
    ...
    fig = px.bar(
        ...
        labels={
            "cuisines": "Type de cuisine",
            "value": "Rapport Qualité-Prix moyen",
...
def box_plot_service(request):
    ...
    melted_df = pd.melt(
        ...
        id_vars=["price_level"],
        value_vars=["service"],
        var_name="Niveau de Satisfaction",
        value_name="Note",
    )
...
def noteMoyenneNbreRestau(request):
    ...
    df["minimum_range"] = pd.to_numeric(
        df["price_range"].to_string().split("-")[0].replace("€", "").replace(",", ""),
        errors="coerce",
    )
    df["maximum_range"] = pd.to_numeric(
        df["price_range"].to_string().split("-")[1].replace("€", "").replace(",", ""),
        errors="coerce",
    )
    ...
    agg_countries_df = (
        ...
        .agg(
            ...
            median_reviews_n=pd.NamedAgg(
                column="total_reviews_count", aggfunc=np.median
            ),
            mean_price=pd.NamedAgg(column="avg_price", aggfunc=np.mean),
            median_price=pd.NamedAgg(column="avg_price", aggfunc=np.median),
            open_days_per_week=pd.NamedAgg(
                column="open_days_per_week", aggfunc=np.mean
            ),
            open_hours_per_week=pd.NamedAgg(
                column="open_hours_per_week", aggfunc=np.mean
            ),
            working_shifts_per_week=pd.NamedAgg(
                column="working_shifts_per_week", aggfunc=np.mean
            ),
        )
        .reset_index(level=0)
    ...
    agg_countries_df["country_code"] = (
        agg_countries_df["country"]
        .map(countries_dict)
        .fillna(agg_countries_df["country"])
    )

    # Bubble plot with the relationship between total_votes and avg_vote for the European countries
    fig = go.Figure(
        data=go.Scatter(
            x=agg_countries_df["total_restaurants"],
            y=agg_countries_df["mean_rating"],
            mode="markers+text",
            marker=dict(
                size=(agg_countries_df["median_reviews_n"].astype("float64") * 0.09),
                color=agg_countries_df["median_reviews_n"],
            ),
            text=agg_countries_df["country"],
...
def radar_chart(request):
    ...
    top8_countries_df = top8_countries_df.melt(
        id_vars=["Country"],
    ...
    decimal_val = 1
    max_countries_val = round_decimals_up_or_down(
        direction="up", number=top8_countries_df["AggValue"].max(), decimals=decimal_val
    )
    min_countries_val = round_decimals_up_or_down(
        direction="down",
        number=top8_countries_df["AggValue"].min(),
        decimals=decimal_val,
    )
    ...
    fig = px.line_polar(
        top8_countries_df,
        r="AggValue",
        range_r=[min_countries_val, max_countries_val],
        theta="Category",
        color="Country",
        line_close=True,
        template="seaborn",
    ...
    fig.update_layout(
        title_text="Notes Globales du top 8",
        title_x=0.5,
        title_y=0.97,
